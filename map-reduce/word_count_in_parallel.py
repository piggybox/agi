...
def map_reduce(documents, mapper_func, reducer_func):
    ...
    """
    # Step 1: Map phase (parallelized)
    mapped = []

    def map_one(item):
        doc_id, text = item
        return mapper_func(doc_id, text)

    with concurrent.futures.ThreadPoolExecutor(max_workers=4) as executor:
        results = list(executor.map(map_one, documents.items()))
    ...
    # Step 3: Reduce phase (parallelized)
    reduced = {}

    def reduce_one(item):
        word, counts = item
        return reducer_func(word, counts)

    with concurrent.futures.ThreadPoolExecutor(max_workers=4) as executor:
        reduce_results = list(executor.map(reduce_one, shuffled.items()))
        for word, total in reduce_results:
            reduced[word] = (word, total)
